...
class ImageResizeServiceTest {
	...
	class CreateSmallImageTest {
		...
		void createSmallImage_Success() {
			...
			ResponseInputStream<GetObjectResponse> testImageStream = createMockResponseInputStream(
					createTestJpgImage(512, 512));
	...
	class ErrorHandlingTest {
		...
		void createSmallImage_ImageConversionFailure() throws IOException {
			// given - 잘못된 이미지 데이터
			ResponseInputStream<GetObjectResponse> invalidImageStream = createMockResponseInputStream(
					new ByteArrayInputStream("invalid image data".getBytes()));
