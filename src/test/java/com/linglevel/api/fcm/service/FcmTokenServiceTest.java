...
class FcmTokenServiceTest {
	...
	void testDeactivateTokenByDevice() {
		...
		when(fcmTokenRepository.findByUserIdAndDeviceId(userId, deviceId)).thenReturn(Optional.of(token));
	...
	void testDeactivateAllTokens() {
		...
		when(fcmTokenRepository.findByUserIdAndIsActive(userId, true)).thenReturn(tokens);
