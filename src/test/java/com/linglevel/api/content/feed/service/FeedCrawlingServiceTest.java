...
class FeedCrawlingServiceTest {
	...
	void extractThumbnailsFromRss() throws Exception {
		// given: 다양한 RSS 피드들
		String[] rssUrls = { "https://feeds.bbci.co.uk/news/technology/rss.xml", "https://techcrunch.com/feed/",
		...
		for (String rssUrl : rssUrls) {
			try {
				...
				long thumbnailCount = entries.stream()
					...
					.filter(entry -> entry.getEnclosures()
						.stream()
				...
				System.out
					.println("Thumbnail Rate: " + String.format("%.1f%%", (thumbnailCount * 100.0 / entries.size())));
				System.out.println();
	...
	void extractDescriptionFromMultipleSources() throws Exception {
		// given: 다양한 RSS 피드들
		String[][] rssSources = { { "BBC", "https://feeds.bbci.co.uk/news/technology/rss.xml" },
		...
		for (String[] source : rssSources) {
			...
			try {
				...
				long withStandardDesc = entries.stream()
					.filter(entry -> entry.getDescription() != null && entry.getDescription().getValue() != null
