...
class UserPreferenceAggregationSchedulerTest {
	...
	void skipAggregationWhenNoLogs() {
		// Given
		when(contentAccessLogRepository.findByAccessedAtAfter(any(Instant.class))).thenReturn(new ArrayList<>());
	...
	void setPrimaryCategoryForArticleLogsOnly() {
		...
		List<ContentAccessLog> logs = List.of(
				...
				createLog(userId, "article3", ContentType.ARTICLE, ContentCategory.BUSINESS,
	...
	void applyTimeDecayWeighting() {
		...
		List<ContentAccessLog> logs = List.of(
				...
				// 7~30일 - 가중치 0.5
				createLog(userId, "article2", ContentType.ARTICLE, ContentCategory.BUSINESS,
						now.minus(15, ChronoUnit.DAYS)),
				createLog(userId, "article3", ContentType.ARTICLE, ContentCategory.BUSINESS,
						now.minus(20, ChronoUnit.DAYS)),
				// 30일 이전 - 가중치 0.2
				createLog(userId, "article4", ContentType.ARTICLE, ContentCategory.SPORTS,
		...
		verify(userCategoryPreferenceRepository).save(argThat(preference -> {
			...
			// 정규화 후: TECH = 1.0/2.2 = 0.45, BUSINESS = 1.0/2.2 = 0.45, SPORTS = 0.2/2.2 =
			// 0.09
	...
	void updateExistingPreference() {
		...
		List<ContentAccessLog> newLogs = List.of(createLog(userId, "article1", ContentType.ARTICLE,
