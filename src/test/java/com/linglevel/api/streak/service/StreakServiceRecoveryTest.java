...
class StreakServiceRecoveryTest {
	...
	void recoverStreak_FreezeUsedDay_ConvertsToCompletedAndRewardsFreeze() {
		...
		List<FreezeTransaction> rewards = allFreezes.stream().filter(tx -> tx.getAmount() == 1).toList();
	...
	void recoverStreak_AfterRecovery_StopsWhenNoFreeze() {
		...
		boolean hasDay3 = allSaved.stream().anyMatch(dc -> dc.getCompletionDate().equals(day3));
	...
	void recoverStreak_MultipleFreeze_ComplexScenario() {
		...
		assertThat(allSaved.stream()
			.anyMatch(dc -> dc.getCompletionDate().equals(day2) && dc.getStreakStatus() == StreakStatus.COMPLETED
					&& dc.getStreakCount() == 2))
			.isTrue();

		// day3: MISSED → COMPLETED (streak=3)
		assertThat(allSaved.stream()
			.anyMatch(dc -> dc.getCompletionDate().equals(day3) && dc.getStreakStatus() == StreakStatus.COMPLETED
					&& dc.getStreakCount() == 3))
			.isTrue();

		// day4: 프리즈 사용으로 FREEZE_USED (streak=3 유지)
		assertThat(allSaved.stream()
			.anyMatch(dc -> dc.getCompletionDate().equals(day4) && dc.getStreakStatus() == StreakStatus.FREEZE_USED
					&& dc.getStreakCount() == 3))
			.isTrue();

		// day5, day6, day7: streakCount 재계산
		assertThat(allSaved.stream().anyMatch(dc -> dc.getCompletionDate().equals(day5) && dc.getStreakCount() == 4))
			.isTrue();

		assertThat(allSaved.stream().anyMatch(dc -> dc.getCompletionDate().equals(day6) && dc.getStreakCount() == 5))
			.isTrue();

		assertThat(allSaved.stream().anyMatch(dc -> dc.getCompletionDate().equals(day7) && dc.getStreakCount() == 6))
			.isTrue();
	...
	private void setupMocks() {
		...
		lenient().when(dailyCompletionRepository.findByUserIdAndCompletionDateGreaterThanEqualOrderByCompletionDateAsc(
			...
			.thenAnswer(invocation -> {
				...
				List<DailyCompletion> sorted = new ArrayList<>(completionMap.values());
				sorted.sort((a, b) -> a.getCompletionDate().compareTo(b.getCompletionDate()));
				return sorted.stream().filter(dc -> !dc.getCompletionDate().isBefore(startDate)).toList();
			});

		// saveAll mock
		lenient().when(dailyCompletionRepository.saveAll(anyList())).thenAnswer(invocation -> {
