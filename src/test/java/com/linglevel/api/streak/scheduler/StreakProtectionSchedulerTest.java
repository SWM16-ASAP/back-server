...
class StreakProtectionSchedulerTest {
	...
	void languageConversion_Korean() throws Exception {
		...
		// then
		verify(fcmMessagingService).sendMessage(eq("token1"),
				argThat(request -> request.getTitle().contains("불꽃") || request.getTitle().contains("스트릭")
						|| request.getTitle().contains("마지막") || request.getTitle().contains("늦지")
						|| request.getTitle().contains("자기") || request.getTitle().contains("남았")
						|| request.getTitle().contains("기다려") || request.getTitle().contains("기회")
						|| request.getTitle().contains("거의") || request.getTitle().contains("오늘")));
	}
	...
	void languageConversion_English() throws Exception {
		...
		// then
		verify(fcmMessagingService).sendMessage(eq("token1"),
				argThat(request -> request.getTitle().contains("flame") || request.getTitle().contains("streak")
						|| request.getTitle().contains("chance") || request.getTitle().contains("late")
						|| request.getTitle().contains("minutes") || request.getTitle().contains("left")
						|| request.getTitle().contains("waiting") || request.getTitle().contains("Almost")
	...
	void languageConversion_Japanese() throws Exception {
		...
		// then
		verify(fcmMessagingService).sendMessage(eq("token1"),
				argThat(request -> request.getTitle().contains("炎") || request.getTitle().contains("ストリーク")
						|| request.getTitle().contains("チャンス") || request.getTitle().contains("遅く")
						|| request.getTitle().contains("寝る前") || request.getTitle().contains("残って")
						|| request.getTitle().contains("待って") || request.getTitle().contains("もうすぐ")
	...
	void sendProtectionMessage_WhenNoFreezeUsed() throws Exception {
		...
		verify(fcmMessagingService).sendMessage(eq("token1"), argThat(request -> {
			...
			String title = request.getTitle();
			String body = request.getBody();
			return (title.contains("자기") || title.contains("남았") || title.contains("기다려") || title.contains("늦지")
					|| title.contains("기회") || title.contains("불꽃") || title.contains("거의") || title.contains("마무리")
					|| title.contains("스트릭")) && (!body.contains("프리즈")); // 프리즈 언급 없음
		}));
	}
