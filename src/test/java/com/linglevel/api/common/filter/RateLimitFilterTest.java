...
class RateLimitFilterTest extends AbstractRedisTest {
	...
	void setUp() throws Exception {
		...
		when(rateLimitResolver.resolveRateLimit(any())).thenReturn(null); // No annotation
																			// (default)
	...
	void testCustomAutoRateLimitWithAuthentication() throws Exception {
		// Given: Mock annotation with 15 requests per minute, AUTO (with authenticated
		// user)
		RateLimit annotation = mock(RateLimit.class);
	...
	void testAnnotationOverridesGlobalConfig() throws Exception {
		...
		// Then: 4th request should be rate limited (not 101st, proving annotation
		// overrides global)
		rateLimitFilter.doFilter(request, response, filterChain);
