...
class TicketTransactionRepositoryTest extends AbstractDatabaseTest {
	...
	void 사용자ID로_거래내역조회_성공() {
		// given
		TicketTransaction transaction1 = createTransaction(testUserId, -5, "Transaction 1",
				TransactionStatus.CONFIRMED);
		TicketTransaction transaction2 = createTransaction(testUserId, 10, "Transaction 2",
				TransactionStatus.CONFIRMED);
		TicketTransaction transaction3 = createTransaction(testUserId, -3, "Transaction 3", TransactionStatus.RESERVED);
		TicketTransaction transaction4 = createTransaction(otherUserId, -2, "Other user transaction",
				TransactionStatus.CONFIRMED);
	...
	void 예약ID와상태로_거래조회_성공() {
		...
		// when
		Optional<TicketTransaction> result = ticketTransactionRepository.findByReservationIdAndStatus(testReservationId,
	...
	void 페이징처리_확인() {
		// given
		for (int i = 1; i <= 25; i++) {
			TicketTransaction transaction = createTransaction(testUserId, -i, "Transaction " + i,
					TransactionStatus.CONFIRMED);
	...
	void 거래상태별_필터링_확인() {
		// given
		TicketTransaction confirmedTransaction = createTransaction(testUserId, -5, "Confirmed",
				TransactionStatus.CONFIRMED);
		TicketTransaction reservedTransaction = createTransaction(testUserId, -3, "Reserved",
				TransactionStatus.RESERVED);
		TicketTransaction cancelledTransaction = createTransaction(testUserId, -2, "Cancelled",
	...
	private TicketTransaction createTransaction(String userId, int amount, String description,
