...
public class FcmMessagingService {
	...
	public BatchResponse sendMulticastMessage(List<String> fcmTokens, FcmMessageRequest messageRequest) {
		...
		try {
			...
			Map<Integer, String> indexToPushId = new java.util.HashMap<>(); // 인덱스별 pushId
																			// 매핑
			...
			log.info("Batch messages sent to {} tokens - Success: {}, Failed: {} (Analytics: {})", fcmTokens.size(),
	...
	private void savePushLogsBatch(List<String> fcmTokens, String campaignGroup, BatchResponse response,
			Map<Integer, String> indexToPushId) {
		try {
			Map<String, String> tokenToUserId = getTokenToUserIdMap(fcmTokens);
	 ...
	 * PushLog 객체 생성
	 */
	private PushLog createPushLog(String pushId, String userId, boolean success, String campaignGroup,
			String fcmMessageId, LocalDateTime now) {
	...
	private Map<String, String> buildDataWithUserId(FcmMessageRequest messageRequest, String userId) {
		Map<String, String> data = messageRequest.getData() != null ? new java.util.HashMap<>(messageRequest.getData())
