...
public class NotificationBroadcastRequest {
	...
	@Schema(description = "국가별 메시지 맵 (최소 US 필수)",
			example = "{\"KR\": {\"title\": \"새 기능\", \"body\": \"확인하세요\"}, \"US\": {\"title\": \"New Feature\", \"body\": \"Check it out\"}}",
			required = true)
	@NotEmpty(message = "Messages are required")
	@Valid
	private Map<String, LocalizedMessage> messages;

	@Schema(description = "커스텀 데이터 (딥링크, 액션 등)",
			example = "{\"deeplink\": \"/announcements/123\", \"action\": \"open_announcement\"}")
