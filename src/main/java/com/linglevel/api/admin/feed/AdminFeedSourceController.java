...
public class AdminFeedSourceController {
	...
	public ResponseEntity<List<FeedSourceResponse>> getAllFeedSources() {
		...
		List<FeedSourceResponse> responses = feedSources.stream().map(this::mapToResponse).collect(Collectors.toList());
	...
	@ApiResponses(value = { @ApiResponse(responseCode = "200", description = "크롤링 성공", useReturnTypeSchema = true),
			@ApiResponse(responseCode = "401", description = "인증 실패",
					content = @Content(schema = @Schema(implementation = ExceptionResponse.class))) })
