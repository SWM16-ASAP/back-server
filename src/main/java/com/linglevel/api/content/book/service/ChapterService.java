...
public class ChapterService {
	...
	public PageResponse<ChapterResponse> getChapters(String bookId, GetChaptersRequest request, String userId) {
		...
		Map<String, Map<DifficultyLevel, Long>> chunkCountsMap = chunkRepository.findChunkCountsByChapterIds(chapterIds)
			.stream()
			.collect(Collectors.groupingBy(ChunkCountByLevelDto::getChapterId,
		...
		List<ChapterResponse> chapterResponses = chapters.stream()
			.map(chapter -> convertToChapterResponse(chapter, book, bookProgress, chunkCountsMap))
	...
	public ChapterResponse getChapter(String bookId, String chapterId, String userId) {
		...
		Map<String, Map<DifficultyLevel, Long>> chunkCountsMap = chunkRepository
			.findChunkCountsByChapterIds(Collections.singletonList(chapterId))
			.stream()
			.collect(Collectors.groupingBy(ChunkCountByLevelDto::getChapterId,
					Collectors.toMap(ChunkCountByLevelDto::getDifficultyLevel, ChunkCountByLevelDto::getCount)));

		return convertToChapterResponse(chapter, book, bookProgress, chunkCountsMap);
	}
	...
	public ChapterNavigationResponse getChapterNavigation(String bookId, String chapterId) {
		...
		Optional<Chapter> previousChapter = chapterRepository.findByBookIdAndChapterNumber(bookId,
				currentChapter.getChapterNumber() - 1);

		Optional<Chapter> nextChapter = chapterRepository.findByBookIdAndChapterNumber(bookId,
				currentChapter.getChapterNumber() + 1);
	...
	private ChapterResponse convertToChapterResponse(Chapter chapter, Book book, BookProgress bookProgress,
			Map<String, Map<DifficultyLevel, Long>> chunkCountsMap) {
		int currentReadChunkNumber = 0;
		double progressPercentage = 0.0;
		DifficultyLevel currentDifficultyLevel = book.getDifficultyLevel(); // Fallback:
																			// Book's
																			// difficulty
