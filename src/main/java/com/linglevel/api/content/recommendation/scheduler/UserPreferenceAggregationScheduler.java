...
public class UserPreferenceAggregationScheduler {
		...
		try {
			...
			for (Entry<String, List<ContentAccessLog>> entry : logsByUser.entrySet()) {
				...
				try {
					...
					failureCount++;
					log.error("Failed to update preference for user: {}, logs count: {}", userId, userLogs.size(), e);
				}
			}
			...
			log.info("User preference aggregation completed. Success: {}, Failure: {}, Duration: {}ms", successCount,
					failureCount, durationMillis);
			...
			log.error("Critical error during user preference aggregation. Success: {}, Failure: {}", successCount,
					failureCount, e);
		}
	}
	...
	private void updateUserPreference(String userId, List<ContentAccessLog> logs) {
		...
		if (!categoryScores.isEmpty()) {
			primaryCategory = categoryScores.entrySet()
				.stream()
		...
		if (primaryCategory != null) {
			log.debug("Updated preference for user {}: primaryCategory={}, totalAccess={}", userId, primaryCategory,
					logs.size());
		}
		else {
	...
	/**
	 * 읽기 시간 기반 가중치 계산 읽기 시간이 길수록 콘텐츠에 대한 진정한 관심도가 높다고 판단
	 */
	...
	private void deleteOldLogs(Instant cutoffDate) {
		try {
			Instant deleteBeforeDate = cutoffDate.minus(30, java.time.temporal.ChronoUnit.DAYS); // 120일
																									// 이전
																									// 로그
																									// 삭제
