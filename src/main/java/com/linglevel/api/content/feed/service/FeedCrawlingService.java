...
public class FeedCrawlingService {
	...
	public int crawlFeedSource(FeedSource feedSource) {
		try {
			...
			log.info("RSS crawling completed: {} feeds collected, {} filtered (soft-deleted) from {}", crawledCount,
					filteredCount, feedSource.getName());
			return crawledCount;
	...
	/**
	 * RSS Entry에서 썸네일 URL 추출 (RSS -> DSL fallback) 1. RSS enclosures에서 이미지 찾기 2. Media
	 * 모듈에서 썸네일 찾기 (YouTube 등) 3. 실패하면 coverImageDsl을 사용하여 article 페이지에서 크롤링
	 */
	private String extractThumbnailUrl(SyndEntry entry, String articleUrl, FeedSource feedSource) {
		...
		if (entry.getEnclosures() != null && !entry.getEnclosures().isEmpty()) {
			String rssThumbnail = entry.getEnclosures()
				.stream()
			...
			if (entry.getForeignMarkup() != null) {
				...
				for (Object element : entry.getForeignMarkup()) {
					if (element instanceof org.jdom2.Element) {
						...
						log.debug("Element name: {}, namespace: {}, namespaceURI: {}", elem.getName(),
								elem.getNamespace(), elem.getNamespaceURI());
		...
		if (feedSource.getCoverImageDsl() != null && !feedSource.getCoverImageDsl().trim().isEmpty()) {
			try {
				...
				Document doc = Jsoup.connect(articleUrl).timeout(10000).userAgent("Mozilla/5.0").get();
	...
	/**
	 * RSS Entry에서 설명 추출 1. RSS description 필드 (일반 RSS) 2. media:description (YouTube 등)
	 * 3. contents 필드
	 */
					...
					if (element instanceof org.jdom2.Element) {
						...
						if ("group".equals(elem.getName()) && elem.getNamespaceURI() != null
							...
							if (descriptionElem != null && descriptionElem.getText() != null) {
								...
								if (!description.isEmpty()) {
									log.debug("Description found in media module: {}",
											description.substring(0, Math.min(50, description.length())));
									description = description.replaceAll("[\\n\\r]", " ").trim();
									// 너무 긴 경우 일부만 추출 (500자 제한)
									return description.length() > 500 ? description.substring(0, 500) + "..."
											: description;
								}
							}
								...
								if (child instanceof org.jdom2.Element) {
									...
									if ("description".equals(childElem.getName())) {
										...
										if (description != null && !description.trim().isEmpty()) {
											...
											log.debug("Description found via children search: {}",
													description.substring(0, Math.min(50, description.length())));
											description = description.replaceAll("[\\n\\r]", " ").trim();
											// 너무 긴 경우 일부만 추출 (500자 제한)
											return description.length() > 500 ? description.substring(0, 500) + "..."
