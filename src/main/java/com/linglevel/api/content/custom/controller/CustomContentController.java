...
public class CustomContentController {
	...
	@Operation(summary = "커스텀 콘텐츠 목록 조회",
	...
	@Operation(summary = "단일 커스텀 콘텐츠 조회", description = "특정 커스텀 콘텐츠의 상세 정보를 조회합니다.")
	@ApiResponses(value = { @ApiResponse(responseCode = "200", description = "조회 성공", useReturnTypeSchema = true),
	...
	public ResponseEntity<CustomContentResponse> getCustomContent(@AuthenticationPrincipal JwtClaims claims,
			@Parameter(description = "커스텀 콘텐츠 ID",
					example = "60d0fe4f5311236168a109ca") @PathVariable String customContentId) {

		CustomContentResponse response = customContentService.getCustomContent(claims.getId(), customContentId);
		return ResponseEntity.ok(response);
	}

	@Operation(summary = "커스텀 콘텐츠 수정", description = "커스텀 콘텐츠의 제목이나 태그를 수정합니다.")
	...
	public ResponseEntity<CustomContentResponse> updateCustomContent(@AuthenticationPrincipal JwtClaims claims,
			@Parameter(description = "커스텀 콘텐츠 ID",
					example = "60d0fe4f5311236168a109ca") @PathVariable String customContentId,
			@RequestBody UpdateCustomContentRequest request) {

		CustomContentResponse response = customContentService.updateCustomContent(claims.getId(), customContentId,
				request);
		return ResponseEntity.ok(response);
	}

	@Operation(summary = "커스텀 콘텐츠 청크 목록 조회", description = "특정 커스텀 콘텐츠에 속한 텍스트 청크(Chunk)들을 난이도별로 조회합니다.")
	@ApiResponses(value = { @ApiResponse(responseCode = "200", description = "조회 성공", useReturnTypeSchema = true),
			@ApiResponse(responseCode = "404", description = "커스텀 콘텐츠를 찾을 수 없음",
					content = @Content(schema = @Schema(implementation = ExceptionResponse.class))),
			@ApiResponse(responseCode = "403", description = "권한 없음",
	...
	public ResponseEntity<PageResponse<CustomContentChunkResponse>> getCustomContentChunks(
		...
		if (claims != null) {
			readingSessionService.startReadingSession(claims.getId(), ContentType.CUSTOM, customContentId);
		}

		PageResponse<CustomContentChunkResponse> response = customContentChunkService
			.getCustomContentChunks(claims.getId(), customContentId, request);
		return ResponseEntity.ok(response);
	}

	@Operation(summary = "단일 커스텀 콘텐츠 청크 조회", description = "특정 커스텀 콘텐츠 청크의 상세 정보를 조회합니다.")
	@ApiResponses(value = { @ApiResponse(responseCode = "200", description = "조회 성공", useReturnTypeSchema = true),
	...
	public ResponseEntity<CustomContentChunkResponse> getCustomContentChunk(@AuthenticationPrincipal JwtClaims claims,
			@Parameter(description = "커스텀 콘텐츠 ID",
					example = "60d0fe4f5311236168a109ca") @PathVariable String customContentId,
			@Parameter(description = "청크 ID", example = "60d0fe4f5311236168a109cd") @PathVariable String chunkId) {

		CustomContentChunkResponse response = customContentChunkService.getCustomContentChunk(claims.getId(),
				customContentId, chunkId);
		return ResponseEntity.ok(response);
	}

	@Operation(summary = "커스텀 콘텐츠 삭제",
			description = "사용자가 본인이 생성한 커스텀 콘텐츠를 삭제합니다. 콘텐츠와 관련된 모든 청크 데이터도 함께 soft delete 처리됩니다.")
	@ApiResponses(value = { @ApiResponse(responseCode = "200", description = "삭제 성공", useReturnTypeSchema = true),
			@ApiResponse(responseCode = "404", description = "커스텀 콘텐츠를 찾을 수 없음",
					content = @Content(schema = @Schema(implementation = ExceptionResponse.class))),
	...
	public ResponseEntity<MessageResponse> deleteCustomContent(@AuthenticationPrincipal JwtClaims claims,
			@Parameter(description = "커스텀 콘텐츠 ID",
					example = "60d0fe4f5311236168a109ca") @PathVariable String customContentId) {
