...
public class CustomContentRepositoryImpl implements CustomContentRepositoryCustom {
	...
	private List<String> getNotStartedContentIds(String userId) {
		...
		// 진도가 없는 콘텐츠만 반환
		return allContentIds.stream().filter(contentId -> !progressContentIds.contains(contentId)).toList();
	}
	...
	public Page<CustomContent> findCustomContentsByUserWithFilters(String userId, GetCustomContentsRequest request,
		...
		operations.add(Aggregation.addFields()
			...
			.withValueOf(org.springframework.data.mongodb.core.aggregation.ConvertOperators.ToObjectId
				.toObjectId("$customContentId"))
			.build());

		// 3. CustomContent와 조인 (lookup) - ObjectId로 변환된 필드 사용
		operations.add(Aggregation.lookup("customContents", // from collection
		...
		if (request.getProgress() != null) {
			...
			List<org.bson.types.ObjectId> objectIds = contentIds.stream().map(org.bson.types.ObjectId::new).toList();
