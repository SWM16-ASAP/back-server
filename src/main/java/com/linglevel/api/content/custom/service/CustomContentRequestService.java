...
public class CustomContentRequestService {
	...
	private Optional<CustomContent> checkCachedContent(String originUrl) {
		...
		Optional<CustomContent> existingContent = customContentRepository
			.findByOriginUrlAndIsDeletedFalse(normalizedUrl);

		if (existingContent.isPresent()) {
			log.info("Cache HIT: Found existing content for URL: {} -> Content ID: {}", normalizedUrl,
					existingContent.get().getId());
		}
		else {
	...
	/**
	 * URL 기반 캐싱이 가능한 ContentType인지 확인 TEXT, PDF는 사용자가 직접 입력한 고유 콘텐츠이므로 캐싱 불가
	 */
