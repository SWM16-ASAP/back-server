...
public class CustomContentProgressController {
	...
	public ResponseEntity<CustomContentReadingProgressResponse> updateProgress(
			@Parameter(description = "커스텀 콘텐츠 ID", example = "60d0fe4f5311236168a109ca") @PathVariable String customId,
			@Valid @RequestBody CustomContentReadingProgressUpdateRequest request,
			@AuthenticationPrincipal JwtClaims claims) {
		CustomContentReadingProgressResponse response = customContentReadingProgressService.updateProgress(customId,
				request, claims.getId());
	...
	public ResponseEntity<CustomContentReadingProgressResponse> getProgress(
			@Parameter(description = "커스텀 콘텐츠 ID", example = "60d0fe4f5311236168a109ca") @PathVariable String customId,
			@AuthenticationPrincipal JwtClaims claims) {
		CustomContentReadingProgressResponse response = customContentReadingProgressService.getProgress(customId,
				claims.getId());
		return ResponseEntity.ok(response);
	}
