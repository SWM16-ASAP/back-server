...
public class CustomContentRequestController {
	...
	@Operation(summary = "콘텐츠 처리 요청 생성", description = "사용자가 텍스트를 입력하여 AI 콘텐츠 처리 요청을 생성합니다.")
	@ApiResponses(value = { @ApiResponse(responseCode = "201", description = "요청 생성 성공", useReturnTypeSchema = true),
	...
	public ResponseEntity<CreateContentRequestResponse> createContentRequest(
			@Parameter(description = "콘텐츠 처리 요청 생성",
					required = true) @Valid @RequestBody CreateContentRequestRequest request,
			@AuthenticationPrincipal JwtClaims claims) {

		CreateContentRequestResponse response = customContentRequestService.createContentRequest(claims.getId(),
				request);
		return ResponseEntity.status(HttpStatus.CREATED).body(response);
	}

	@Operation(summary = "콘텐츠 처리 요청 목록 조회", description = "사용자의 콘텐츠 처리 요청 목록을 조회합니다. 진행 중이거나 완료된 요청들을 상태별로 확인할 수 있습니다.")
	...
	public ResponseEntity<PageResponse<ContentRequestResponse>> getContentRequests(
		...
		PageResponse<ContentRequestResponse> response = customContentRequestService.getContentRequests(claims.getId(),
				request);
		return ResponseEntity.ok(response);
	}

	@Operation(summary = "특정 콘텐츠 처리 요청 조회", description = "특정 콘텐츠 처리 요청의 상세 정보를 조회합니다.")
	@ApiResponses(value = { @ApiResponse(responseCode = "200", description = "조회 성공", useReturnTypeSchema = true),
	...
	public ResponseEntity<ContentRequestResponse> getContentRequest(
			@Parameter(description = "요청 ID", example = "60d0fe4f5311236168a109ca") @PathVariable String requestId,
			@AuthenticationPrincipal JwtClaims claims) {
