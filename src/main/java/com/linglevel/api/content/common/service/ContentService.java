...
public class ContentService {
	...
	private record GenericProgress(String contentId, ContentType contentType, Instant lastStudiedAt,
		...
		List<RecentContentResponse> result = paginatedProgresses.stream().map(p -> {
			switch (p.contentType()) {
				case BOOK: {
					...
					// [FIX] Use normalizedProgress for accurate percentage. Fallback to
					// old calculation if null.
					...
					return RecentContentResponse.builder()
						...
						.difficultyLevel(book.getDifficultyLevel().name())
						.tags(book.getTags())
				...
				case ARTICLE: {
					...
					// This part is kept for now, but the percentage is taken from the
					// migrated data.
					try {
					...
					return RecentContentResponse.builder()
						...
						.difficultyLevel(article.getDifficultyLevel().name())
				...
				case CUSTOM: {
					...
					return RecentContentResponse.builder()
						...
						.coverImageUrl(custom.getCoverImageUrl())
						.difficultyLevel(custom.getDifficultyLevel().name())
						.tags(custom.getTags())
						.readingTime(custom.getReadingTime())
						.chunkCount((int) totalChunks)
						.currentReadChunkNumber(currentChunkNumber)
						.progressPercentage(progressPercentage)
						.isCompleted(progress.getIsCompleted())
						.originUrl(custom.getOriginUrl())
						.originDomain(custom.getOriginDomain())
						.lastStudiedAt(p.lastStudiedAt())
						.build();
				}
				default:
