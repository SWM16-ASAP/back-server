...
/**
 * 스트릭 보호 알림 스케줄러 매일 밤 9시에 실행하여 스트릭이 깨지지 않도록 알림을 전송합니다. - 조건: currentStreak > 0 && 오늘 학습
 * 미완료
 */
@Component
	...
	public void sendStreakProtectionNotifications() {
		...
		try {
			...
			for (UserStudyReport report : activeUsers) {
				...
				StreakReminderMessage messageType = usedFreezeYesterday ? StreakReminderMessage.STREAK_SAVED_BY_FREEZE
				...
				try {
					if (fcmTokens.size() == 1) {
						...
						notificationsSent++;
						log.debug("[Streak Protection] Sent to user: {} (streak: {}, lang: {}, type: {})", userId,
								report.getCurrentStreak(), languageCode, messageType);
					}
					else {
						...
						for (int i = 0; i < response.getResponses().size(); i++) {
							if (response.getResponses().get(i).isSuccessful()) {
								...
								log.warn("[Streak Protection] Failed to send to user: {}, token error: {}", userId,
										response.getResponses().get(i).getException().getMessage());
								fcmTokenService.deactivateToken(failedToken);
							}
						}

						log.debug("[Streak Protection] Multicast to user: {} - Success: {}, Failed: {}", userId,
			...
			log.info(
					...
					candidateUsers, usersWithoutCompletion, usersWithTokens, notificationsSent, notificationsFailed,
					durationMillis);
