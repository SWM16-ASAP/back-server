...
public class StreakService {
	...
	private void grantFreezeIfEligible(UserStudyReport report, int currentStreak, String userId) {
		boolean isEligible = currentStreak > 0 && currentStreak % FREEZE_REWARD_CYCLE == 0
		...
		log.info("Granted 1 freeze to user {} for {} day streak. User now has {} freezes.", userId, currentStreak,
				report.getAvailableFreezes());
	}
	...
	private void grantTicketIfEligible(int currentStreak, String userId) {
		...
		log.info("Granted {} ticket to user {} for {} day streak.", TICKET_REWARD_AMOUNT, userId, currentStreak);
	}
	...
	public boolean hasCompletedStreakToday(String userId, LocalDate today) {
		return dailyCompletionRepository.findByUserIdAndCompletionDate(userId, today)
	...
	private double calculatePercentile(UserStudyReport report) {
		...
		long usersWithHigherOrEqualStreak = userStudyReportRepository
			.countByCurrentStreakGreaterThanEqual(report.getCurrentStreak());
	...
	private EncouragementMessage getEncouragementMessage(int currentStreak, StreakStatus todayStatus,
	...
	private StreakStatus calculateTodayStatus(String userId, LocalDate date) {
		DailyCompletion completion = dailyCompletionRepository.findByUserIdAndCompletionDate(userId, date)
	...
	public Page<FreezeTransactionResponse> getFreezeTransactions(String userId, int page, int limit) {
		...
		Page<FreezeTransaction> transactions = freezeTransactionRepository.findByUserIdOrderByCreatedAtDesc(userId,
				pageRequest);
	...
	private boolean hasFreezeTransaction(String userId, LocalDate date) {
		...
		return freezeTransactionRepository.existsByUserIdAndAmountAndCreatedAtBetween(userId, -1, dayStart, dayEnd);
	}
	...
	private CalendarViewData prepareCalendarViewData(String userId, LocalDate startDate, LocalDate endDate) {
		...
		completions = completions.stream().map(this::ensureStreakStatus).toList();
	...
	@Transactional
	protected void backfillMissingStreakCountsInRange(String userId, LocalDate startDate, LocalDate endDate,
		...
		for (LocalDate currentDate = startDate; !currentDate.isAfter(endDate); currentDate = currentDate.plusDays(1)) {
			...
			Map<LocalDate, DailyCompletion> extendedCompletionMap = loadExtendedCompletions(userId, streakStartDate,
					endDate);

			List<DailyCompletion> updates = calculateAndCollectUpdates(streakStartDate, endDate, extendedCompletionMap,
					userId, processedDates);
	...
	private Map<LocalDate, DailyCompletion> loadExtendedCompletions(String userId, LocalDate streakStartDate,
			LocalDate endDate) {
		...
		return extendedCompletions.stream().collect(Collectors.toMap(DailyCompletion::getCompletionDate, c -> c));
	}

	private List<DailyCompletion> calculateAndCollectUpdates(LocalDate startDate, LocalDate endDate,
			Map<LocalDate, DailyCompletion> completionMap, String userId, Set<LocalDate> processedDates) {
		...
		while (!currentDate.isAfter(endDate)) {
			...
			if (hasStreakActivity) {
				...
				if (completion.getStreakCount() == null) {
					...
					log.debug("Scheduled streakCount update for user {} on date {} to {}", userId, currentDate,
							streakCount);
				}
	...
	private void saveUpdatesAndUpdateMap(List<DailyCompletion> toUpdate, Map<LocalDate, DailyCompletion> completionMap,
			String userId) {
	...
	private LocalDate findStreakStartDate(String userId, LocalDate fromDate,
			Map<LocalDate, DailyCompletion> completionMap) {
		...
		while (!currentDate.isBefore(searchLimit)) {
			...
			if (!hasStreakActivity) {
				completion = dailyCompletionRepository.findByUserIdAndCompletionDate(userId, currentDate)
	...
	private CalendarDayInfo calculateCalendarDayInfo(LocalDate date, LocalDate today, CalendarViewData viewData) {
		...
		if (!isFuture) {
			rewards = createRewardInfo(viewData.getTicketRewardsMap().getOrDefault(date, 0),
	...
	private RewardInfo createRewardInfo(int tickets, int freezes) {
		return RewardInfo.builder().tickets(tickets).freezes(freezes).build();
	}
	...
	public UserStudyReport recalculateUserStudyReport(String userId) {
		...
		List<DailyCompletion> allCompletions = dailyCompletionRepository.findByUserIdOrderByCompletionDateAsc(userId);
		...
		log.info("Recalculated UserStudyReport for user {}. Current streak: {}, Longest streak: {}", userId,
	...
	public void recoverStreak(String userId, LocalDate startDate, LocalDate endDate) {
		...
		for (LocalDate date = startDate; !date.isAfter(endDate); date = date.plusDays(1)) {
			Optional<DailyCompletion> existingOpt = dailyCompletionRepository.findByUserIdAndCompletionDate(userId,
					date);
			...
			if (existingOpt.isPresent()) {
				...
				if (existing.getStreakStatus() == StreakStatus.FREEZE_USED) {
					...
					FreezeTransaction rewardTx = FreezeTransaction.builder()
						...
						.createdAt(Instant.now())
						.build();
					freezeTransactions.add(rewardTx);
					earnedFreezes++;

					log.info("Recovered FREEZE_USED to COMPLETED for user {} on {}. Rewarded 1 freeze.", userId, date);
				}
			...
			if (existingOpt.isPresent()) {
				...
				if (existing.getStreakStatus() == StreakStatus.COMPLETED) {
					...
					if (availableFreezes > 0) {
						...
						log.info("Auto-used freeze for user {} on {}. {} freezes remaining.", userId, currentDate,
								availableFreezes);
						...
						// 프리즈 없음 - 연결 중단
						log.info("No freeze available for user {} on {}. Stopping streak connection.", userId,
								currentDate);
						break;
					}
				}
				...
				if (availableFreezes > 0) {
					DailyCompletion newFreezeCompletion = DailyCompletion.builder()
						...
						.createdAt(Instant.now())
						.build();
					completionsToSave.add(newFreezeCompletion);
					...
					log.info("Auto-used freeze and created FREEZE_USED for user {} on {}. {} freezes remaining.",
							userId, currentDate, availableFreezes);

					currentDate = currentDate.plusDays(1);
				}
				else {
					// 프리즈 없음 - 연결 중단
					log.info("No freeze available for user {} on {}. Stopping streak connection.", userId, currentDate);
					break;
				}
			}
		}
		...
		if (!freezeTransactions.isEmpty()) {
			freezeTransactionRepository.saveAll(freezeTransactions);
		...
		log.info(
				"Streak recovery completed for user {} from {} to {}. Earned {} freezes, used {} freezes. Final freezes: {}",
	...
	public void recalculateAllStreakCounts(String userId, LocalDate recoveryStartDate) {
		...
		if (completionsToRecalculate.isEmpty()) {
			log.info("No completions found for user {} from {}. Skipping streakCount recalculation.", userId,
					recoveryStartDate);
			return;
		}
