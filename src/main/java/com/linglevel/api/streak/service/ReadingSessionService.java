...
public class ReadingSessionService {
	...
	public void startReadingSession(String userId, ContentType contentType, String contentId) {
		...
		if (existingSession != null && existingSession.getContentType().equals(contentType)
		...
		log.info("Reading session started - userId: {}, content: {}/{}, redisKey: {}, startedAt: {}", userId,
				contentType.getCode(), contentId, key, Instant.ofEpochMilli(session.getStartedAtMillis()));
	}
	...
	public boolean isReadingSessionValid(String userId, ContentType contentType, String contentId) {
		...
		ReadingSession session = getReadingSession(userId);
		if (session == null) {
			log.info("No reading session found - userId: {}, redisKey: {}, expected content: {}/{}", userId, key,
					contentType.getCode(), contentId);
			return false;
		}

		if (!session.getContentType().equals(contentType) || !session.getContentId().equals(contentId)) {
			log.info("Reading session content mismatch - userId: {}, expected: {}/{}, actual: {}/{}", userId,
					contentType.getCode(), contentId, session.getContentType().getCode(), session.getContentId());
			return false;
		}
		...
		if (readingDuration.compareTo(MIN_READING_DURATION) < 0) {
			log.info("Reading duration too short - userId: {}, duration: {} seconds (minimum: 30)", userId,
					readingDuration.getSeconds());
			return false;
		}

		log.info("Reading session validated successfully - userId: {}, content: {}/{}, duration: {} seconds", userId,
				contentType.getCode(), contentId, readingDuration.getSeconds());
		return true;
	}
