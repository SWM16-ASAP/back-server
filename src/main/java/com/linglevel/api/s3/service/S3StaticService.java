...
public class S3StaticService {
	...
	public void deleteFiles(String contentId, S3PathStrategy pathStrategy) {
		try {
			...
			log.error("Failed to delete files from S3 Static - contentId: {}, prefix: {}, error: {}", contentId,
					pathStrategy.generateBasePath(contentId), e.getMessage());
		...
		try {
			...
			do {
				...
				if (!listResponse.contents().isEmpty()) {
					...
					log.info("Deleted {} objects from S3 Static with prefix: {}", deleteResponse.deleted().size(),
							prefix);
				}
