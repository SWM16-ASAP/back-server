...
public class TicketsController {
	...
	@ApiResponses({ @ApiResponse(responseCode = "200", description = "잔고 조회 성공", useReturnTypeSchema = true),
	...
	@ApiResponses({ @ApiResponse(responseCode = "200", description = "거래 내역 조회 성공", useReturnTypeSchema = true),
			@ApiResponse(responseCode = "401", description = "인증 실패",
					content = @Content(schema = @Schema(implementation = ExceptionResponse.class))) })
	public ResponseEntity<PageResponse<TicketTransactionResponse>> getTicketTransactions(
		...
		var transactions = ticketService.getTicketTransactions(claims.getId(), request.getPage(), request.getLimit());

		PageResponse<TicketTransactionResponse> response = new PageResponse<>(transactions.getContent(), transactions);
