...
import com.linglevel.api.bookmark.service.BookmarkService;
...
public class BookmarksController {
	...
	public ResponseEntity<PageResponse<BookmarkedWordResponse>> getBookmarkedWords(
		...
		var bookmarkedWords = bookmarkService.getBookmarkedWords(claims.getId(), request.getPage(), request.getLimit(),
				request.getSearch());
	...
	public ResponseEntity<MessageResponse> addWordBookmark(
			@Parameter(description = "북마크할 단어", example = "magnificent") @PathVariable String word,
			@AuthenticationPrincipal JwtClaims claims) {
	...
	public ResponseEntity<MessageResponse> removeWordBookmark(
			@Parameter(description = "북마크 해제할 단어", example = "magnificent") @PathVariable String word,
			@AuthenticationPrincipal JwtClaims claims) {
		String validatedWord = wordValidator.validateAndPreprocess(word);
	...
	@ApiResponses(value = { @ApiResponse(responseCode = "200", description = "토글 성공", useReturnTypeSchema = true),
			@ApiResponse(responseCode = "401", description = "인증 실패",
					content = @Content(schema = @Schema(implementation = ExceptionResponse.class))) })
	@RateLimit(capacity = 30, refillMinutes = 1, keyType = KeyType.USER)
	@PutMapping("/words/{word}/toggle")
	public ResponseEntity<BookmarkToggleResponse> toggleWordBookmark(
			@Parameter(description = "토글할 단어", example = "magnificent") @PathVariable String word,
			@AuthenticationPrincipal JwtClaims claims) {
		String validatedWord = wordValidator.validateAndPreprocess(word);
	...
	public ResponseEntity<BookmarkToggleResponse> toggleWordBookmarkById(
			@Parameter(description = "토글할 단어의 ID", example = "60d0fe4f5311236168a109ca") @PathVariable String wordId,
			@AuthenticationPrincipal JwtClaims claims) {
