...
public class DslParser {
	...
	public ASTNode parse() {
		...
		if (current().getType() != TokenType.EOF) {
			throw new RuntimeException(
					"Unexpected token at position " + current().getPosition() + ": " + current().getValue());
		}
		return result;
	}
	...
	private ASTNode parseChain() {
		...
		while (true) {
			if (current().getType() == TokenType.GT) {
				...
				ASTNode source = items.size() == 1 ? items.get(0)
						: new ChainNode(items.get(0), new ArrayList<>(items.subList(1, items.size())));
				items.clear();
	...
	private List<ASTNode> parseActionChain() {
		...
		if (current().getType() == TokenType.LPAREN) {
			...
			while (current().getType() != TokenType.RPAREN && current().getType() != TokenType.EOF) {
				if (!isPostfix()) {
					throw new RuntimeException(
							"Only postfix operations allowed in > action chain at position " + current().getPosition());
				}
	...
	private boolean isPostfix() {
		return current().getType() == TokenType.SINGLE_QUOTE || current().getType() == TokenType.TRIPLE_QUOTE
				|| current().getType() == TokenType.AT || current().getType() == TokenType.HASH;
	}
	...
	private ASTNode parsePrimary() {
		if (current().getType() == TokenType.D) {
			...
			throw new RuntimeException(
					"Unexpected token at position " + current().getPosition() + ": " + current().getType());
		}
	}
