...
public class CrawlingController {
	...
	public ResponseEntity<DslLookupResponse> lookupDsl(
			@Parameter(description = "크롤링할 전체 URL", required = true,
					example = "https://www.coupang.com/vp/products/123456") @RequestParam String url,
			@Parameter(description = "DSL 반환 없이 유효성만 검증",
					example = "false") @RequestParam(defaultValue = "false") boolean validate_only) {
	...
	@Operation(summary = "등록된 도메인 목록 조회",
			description = "시스템에 등록된 모든 도메인 목록을 조회합니다. contentTypes 파라미터로 특정 콘텐츠 타입들만 필터링할 수 있습니다.")
	@ApiResponses(value = { @ApiResponse(responseCode = "200", description = "조회 성공", useReturnTypeSchema = true) })
	@GetMapping("/crawling-dsl/domains")
	public ResponseEntity<PageResponse<DomainsResponse>> getDomains(
			@Parameter(description = "조회할 페이지 번호", example = "1") @Min(value = 1,
					message = "페이지 번호는 1 이상이어야 합니다.") @RequestParam(defaultValue = "1") int page,
			@Parameter(description = "페이지 당 항목 수 (최대 200)", example = "10") @Min(value = 1,
					message = "페이지 당 항목 수는 1 이상이어야 합니다.") @Max(value = 200,
							message = "페이지 당 항목 수는 200 이하여야 합니다.") @RequestParam(defaultValue = "10") int limit,
			@Parameter(description = "필터링할 콘텐츠 타입 목록 (선택 사항)", example = "[\"BLOG\", \"NEWS\"]") @RequestParam(
					required = false) List<FeedContentType> contentTypes) {
