...
public class RateLimitFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
		...
		if (rateLimitAnnotation != null) {
			...
			log.debug("Using annotation-based rate limit config: capacity={}, refillMinutes={}", config.getCapacity(),
					config.getRefillMinutes());
		}
		else {
			// Fallback to global configuration
			config = RateLimitConfig.fromProperties(rateLimitProperties);
			log.debug("Using global rate limit config: capacity={}, refillMinutes={}", config.getCapacity(),
					config.getRefillMinutes());
		}
