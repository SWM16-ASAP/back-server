 ...
 *
 * 북마크, 원형 조회 등 번역이 필요없는 작업에 사용 - WordVariant는 "변형 → 원형" 매핑만 저장 (언어 정보 없음) - 예: "ran" →
 * "run", "prettiest" → "pretty"
 */
@Service
...
public class WordVariantService {
	...
	public List<String> getOriginalForms(String word) {
		return wordVariantRepository.findAllByWord(word).stream().map(WordVariant::getOriginalForm).distinct().toList();
	}
