...
public class Oxford3000Service {
	...
	public Oxford3000InitResponse initializeOxford3000(LanguageCode targetLanguage) {
			...
			for (int attempt = 1; attempt <= MAX_RETRY_ATTEMPTS; attempt++) {
				try {
					...
					if ((wordsToProcess.size() <= 50 && (i + 1) % 10 == 0)
						...
						log.info("Progress: {}/{} words processed ({} succeeded, {} failed)", i + 1,
								wordsToProcess.size(), successCount, failureCount);
					}
					...
					if (attempt < MAX_RETRY_ATTEMPTS) {
						log.warn("Failed to process word '{}' (attempt {}/{}): {}. Retrying...", word, attempt,
								MAX_RETRY_ATTEMPTS, e.getMessage());
					}
					else {
						log.error("Failed to process word '{}' after {} attempts: {}", word, MAX_RETRY_ATTEMPTS,
								e.getMessage(), e);
					}
				}
			}
		...
		return Oxford3000InitResponse.builder()
			...
			.message(String.format("Processed %d words (%d skipped, %d succeeded, %d failed)", wordsToProcess.size(),
					skippedCount, successCount, failureCount))
			.build();
	}
	...
	public boolean processWord(String word, LanguageCode targetLanguage) {
		...
		Optional<Word> existingWord = wordRepository.findByWordAndTargetLanguageCode(originalForm, targetLanguage);
		...
		if (existed) {
			...
			// 생성된 결과 조회
			wordToUpdate = wordRepository.findByWordAndTargetLanguageCode(originalForm, targetLanguage)
	...
	public List<String> getEssentialWordsList(LanguageCode targetLanguage) {
		...
		for (String originalForm : originalForms) {
			List<WordVariant> variants = wordVariantRepository.findAllByOriginalForm(originalForm);
			variants.stream()
