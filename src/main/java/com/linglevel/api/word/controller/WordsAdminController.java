...
public class WordsAdminController {
	...
	@Operation(summary = "단어 강제 재분석", description = """
			...
			**파라미터:**
			...
			""")
	@ApiResponses(value = { @ApiResponse(responseCode = "200", description = "재분석 성공", useReturnTypeSchema = true),
	...
	public ResponseEntity<WordSearchResponse> forceAnalyzeWord(
			@Parameter(description = "재분석할 단어", example = "saw") @PathVariable String word,
			@Parameter(description = "번역 대상 언어",
					example = "KO") @RequestParam(defaultValue = "KO") LanguageCode targetLanguage,
			@Parameter(description = "true: 기존 데이터 삭제 후 재생성, false: 기존 데이터 유지 + 새로운 의미 추가") @RequestParam(
					defaultValue = "false") boolean overwrite,
			@Parameter(description = "true: Variant도 함께 삭제 (완전 초기화), false: Variant 유지 (기본값)") @RequestParam(
					defaultValue = "false") boolean deleteVariants) {
	...
	@Operation(summary = "Oxford 3000 단어 초기화", description = """
			...
			**파라미터:**
			...
			""")
	@ApiResponses(value = { @ApiResponse(responseCode = "200", description = "초기화 성공", useReturnTypeSchema = true),
			@ApiResponse(responseCode = "401", description = "인증 실패 (관리자 권한 필요)",
					content = @Content(schema = @Schema(implementation = ExceptionResponse.class))) })
	...
	@Operation(summary = "필수 단어 통계 조회", description = """
	...
	@Operation(summary = "등록된 필수 단어 목록 조회", description = """
	...
	@Operation(summary = "단어의 필수 여부 설정", description = """
			...
			**사용 사례:**
			...
			""")
	@ApiResponses(value = { @ApiResponse(responseCode = "200", description = "업데이트 성공"),
			@ApiResponse(responseCode = "404", description = "단어를 찾을 수 없음",
					content = @Content(schema = @Schema(implementation = ExceptionResponse.class))),
			@ApiResponse(responseCode = "401", description = "인증 실패 (관리자 권한 필요)",
					content = @Content(schema = @Schema(implementation = ExceptionResponse.class))) })
