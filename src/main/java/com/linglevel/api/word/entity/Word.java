 ...
 *
 * 같은 단어도 언어 쌍별로 여러 개 저장 가능 예: "run" EN->KO, "run" EN->JA는 별도 문서
 */
@Getter
@Setter
@Builder
...
@CompoundIndexes({ @CompoundIndex(name = "word_target_source_language_unique_idx",
		def = "{'word': 1, 'targetLanguageCode': 1, 'sourceLanguageCode': 1}", unique = true) })
public class Word {
	...
	/**
	 * 원형 단어 (예: "pretty", "see", "run") 복합 unique index의 일부 (word + targetLanguageCode +
	 * sourceLanguageCode)
	 */
	private String word;
