...
요청별 지연 구간은 Grafana의 `Explore`에서 `Tempo`를 선택한 뒤 TraceQL `{ resource.service.name = "llv-api" }`로 조회한다. HTTP 요청 span 아래에서 MongoDB, Redis, HTTP client 등 Java Agent가 지원하는 라이브러리의 child span을 비교한다. 일반 내부 메서드는 자동으로 모두 span이 되지 않으므로, 이 정보로 범위를 좁힌 뒤 필요한 지점에만 custom span을 추가한다.
